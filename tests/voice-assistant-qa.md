...
> MIGRATED 2026-09-06 (post-build): parse-audio now Groq (`GROQ_API_KEY`); TTS is browser Web Speech API (no backend route, no key) — `POST /api/voice/speak`, `GEMINI_API_KEY`, voice `Kore` REMOVED. Rows below referencing them are archive; row 3.15 is superseded by 3.15b.
...
| Backend timeout → "Offline mode" badge + retry | IMPLEMENTED | Upload abort at 40 s: `voiceService.ts:96` `UPLOAD_TIMEOUT_MS = 40000` (covers Groq worst case STT 10s + LLM 12s ×2 with fallback retry = 34s). `AbortError` → `report('timeout', true, MSG_TIMEOUT)`; aborts caused by loop `stop()` stay silent (`!active` guard). `App.tsx`: timeout/server/network → `setOfflineMode(true)` (loop kept alive for retry, no teardown); any 200 chunk clears it. Badge `role="status"`: "Offline mode — voice retries automatically. You can keep talking or type instead.". |
...
| No audio persistence | IMPLEMENTED | `voiceService.ts` contains zero `localStorage`/`IndexedDB`/`sessionStorage`/`cookie` writes. Chunks live in-memory (`pendingBlobs`) and are dropped after enqueue. TTS is browser-local Web Speech API (no audio bytes, no object URL). Repo `localStorage` hits are pre-existing config/history (`storageService.ts`) and `ErrorBoundary`, never audio. |
| `.env` ignored | IMPLEMENTED | `git check-ignore .env` → exit 0 (ignored via `.gitignore`); `git check-ignore .env.example` → exit 1 (template committable, contains empty `GROQ_API_KEY=` + `VITE_API_BASE_URL`; no TTS key needed). `.env` exists locally, is untracked-but-ignored (absent from `git status`). `.env` content never opened by QA. |
| No secret in code | IMPLEMENTED | Grep of `src/` for hardcoded keys → no files. Backend reads the only key from env (`GROQ_API_KEY` in `backend/main.py` `_resolve_groq_auth`); missing key → `503 {error, retryable:false}` naming `GROQ_API_KEY`. Startup log shows `configured`/`MISSING` only (no key material). Provider error detail stays server-side; clients get generic `voice parse failed`. |
...
| 3.15b | Live Groq parse + local speech | BLOCKED | Needs human: copy `.env.example` → `.env`, set real `GROQ_API_KEY` (console.groq.com/keys), run `uvicorn backend.main:app --reload --port 8000`, then `curl -F "audio=@sample.webm" -F "current_step=1" -F "known_json={\"confirmed\":[]}" http://localhost:8000/api/voice/parse-audio` (expect 200 contract JSON); without key expect the verified `503` mentioning `GROQ_API_KEY`. `POST /api/voice/speak` no longer exists (expect 404) — speech is browser-local: in Chrome, allow mic, click bubble, speak, confirm fields fill AND an English voice reads the prompt aloud. Log chunk counts to confirm < $0.05/sim (Groq STT+LLM ≈ $0.005/sim, TTS $0). |
